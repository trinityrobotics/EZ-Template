...
// void Drive::set_exit_condition(PID* pid, int p_small_exit_time, double p_small_error, int p_big_exit_time, double p_big_error, int p_velocity_exit_time, int p_mA_timeout) {
//   pid->set_exit_condition(p_small_exit_time, p_small_error, p_big_exit_time, p_big_error, p_velocity_exit_time, p_mA_timeout);
// }
...
void Drive::set_exit_condition(int type, int p_small_exit_time, double p_small_error, int p_big_exit_time, double p_big_error, int p_velocity_exit_time, int p_mA_timeout) {
  ...
  if (type == heading_exit) {
    headingPID.set_exit_condition(p_small_exit_time, p_small_error, p_big_exit_time, p_big_error, p_velocity_exit_time, p_mA_timeout);
  }
...
void Drive::wait_drive() {
  ...
  // Swing Exit
    ...
    while (swing_exit == RUNNING) {
      // swing_exit = swing_exit != RUNNING ? swing_exit : swingPID.exit_condition();
      swing_exit = swingPID.exit_condition();
      pros::delay(util::DELAY_TIME);
    }
  ...
  // Turn Exit
  else if (mode == GPS_TURN || mode == GPS_DRIVE) {
    exit_output exit_condition = RUNNING;
    while (exit_condition == RUNNING) {
      switch (mode) {
        case GPS_TURN:
          exit_condition = GPSturnPID.exit_condition();
          break;
        case GPS_DRIVE:
          exit_condition = GPSdrivePID.exit_condition();
          break;
        default:
          exit_condition = ERROR_NO_CONSTANTS;
      }
      pros::delay(util::DELAY_TIME);
    }
    if (print_toggle) std::cout << "  Turn: " << exit_to_string(exit_condition) << " Exit.\n";

    if (exit_condition == mA_EXIT || turn_exit == VELOCITY_EXIT) {
      interfered = true;
    }
  }
}
...
void Drive::wait_until(double target) {
  ...
  else if (mode == GPS_DRIVE) {
    // Calculate error between current and target (target needs to be an in between position)
    int l_tar = l_start + (target * TICK_PER_METER);
    int r_tar = r_start + (target * TICK_PER_METER);
    int l_error = l_tar - left_sensor();
    int r_error = r_tar - right_sensor();
    int l_sgn = util::sgn(l_error);
    int r_sgn = util::sgn(r_error);
